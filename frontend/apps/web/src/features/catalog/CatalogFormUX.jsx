import { AlertCircle, Check, ChevronRight, CircleHelp, History, Sparkles, X } from 'lucide-react';
...
export function RecoveryNotice({ draft, onRestore, onDiscard }) {
  if (!draft) return null;
  const when = new Date(draft.savedAt);
  return (
    <aside className="flex flex-wrap items-center gap-3 rounded-2xl border border-violet-200 bg-violet-50 p-3 text-violet-950 shadow-sm" role="status">
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-violet-100"><History className="h-4 w-4" /></span>
      <div className="min-w-48 flex-1"><p className="text-xs font-bold">Unfinished work is available</p><p className="text-[11px] text-violet-700">Autosaved {Number.isNaN(when.getTime()) ? 'recently' : when.toLocaleString('en-IN')} on this device. Server data has not been replaced.</p></div>
      <button type="button" onClick={onDiscard} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-violet-700 transition hover:bg-violet-100"><X className="h-3.5 w-3.5" />Discard</button>
      <button type="button" onClick={onRestore} className="rounded-lg bg-violet-700 px-3 py-1.5 text-xs font-bold text-white shadow-sm transition hover:bg-violet-800">Restore draft</button>
    </aside>
  );
}
